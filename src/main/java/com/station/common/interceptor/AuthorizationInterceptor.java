...
public class AuthorizationInterceptor implements HandlerInterceptor {
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		log.debug("ip:" + HttpClientUtil.getIpAddr(request) + "[" + handler + "] executeTime : " + executeTime + "ms");
		String context = request.getContextPath() + "/";
		log.debug("[" + request.getRequestURI().replace(context, "/") + "] executeTime : " + executeTime + "ms");
		// if(executeTime>1000){
		// log.error("===================================================");
		// log.error("ip:"+HttpClientUtil.getIpAddr(request)+"[" + handler + "]
